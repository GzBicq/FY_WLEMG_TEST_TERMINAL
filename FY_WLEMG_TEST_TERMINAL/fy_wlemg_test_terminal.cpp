...
#define OFFSET(index) (index*108+5)

FY_WLEMG_TEST_TERMINAL::FY_WLEMG_TEST_TERMINAL(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::FY_WLEMG_TEST_TERMINAL),
     m_serial(new QSerialPort(this))
{

    ui->setupUi(this);
    fillPortsInfo();
    connect(m_serial, &QSerialPort::readyRead, this, &FY_WLEMG_TEST_TERMINAL::readData);
}

FY_WLEMG_TEST_TERMINAL::~FY_WLEMG_TEST_TERMINAL()
{
    closeSerialPort();
    delete ui;
}

void FY_WLEMG_TEST_TERMINAL::fillPortsInfo()
{
    ui->CB->clear();
    QString description;
...
void FY_WLEMG_TEST_TERMINAL::openSerialPort()
{
    ...
    {
         ui->TB->insertPlainText(tr("Open success\n"));
    } else
    {
         ui->TB->insertPlainText(tr("Open error\n"));
    }
}

void FY_WLEMG_TEST_TERMINAL:: closeSerialPort()
{
    if (m_serial->isOpen())
        m_serial->close();
     ui->TB->insertPlainText(tr("Disconnected\n"));
}

void FY_WLEMG_TEST_TERMINAL::writeData(const QByteArray &data)
{
    m_serial->write(data);
}
...
uint16_t  FY_WLEMG_TEST_TERMINAL::crc_check(uint8_t* data, uint32_t length)
{
...
void FY_WLEMG_TEST_TERMINAL::readData()
{
    QString str;
...
void FY_WLEMG_TEST_TERMINAL::on_PB_PRINT_clicked()
{

}

void FY_WLEMG_TEST_TERMINAL::on_PB_CLEAR_clicked()
{
     QString str;
    pkt_num = 0;
    pkt_byte_num = 0;
